...
import gen.wzp.main.utils.ActivityForResultUtil;
...
public class AboutActivity extends Activity {
	...
	private UserMoreInfo mUserMoreInfo;// 当前查看的用户的资料数据
	...
	public void about_activity_back(View v) {
		this.finish();
	}
	...
	private void findViewById() {
		...
		mSingle = (TextView) findViewById(R.id.about_single);
		...
		mSingleIcon = (ImageView) findViewById(R.id.about_single_icon);
		...
		mHometownIcon = (ImageView) findViewById(R.id.about_single_icon);
		...
		mGoodatIcon = (ImageView) findViewById(R.id.about_single_icon);
								...
								new DialogInterface.OnClickListener() {
									...
									public void onClick(DialogInterface dialog,
										...
										case 0:
											...
											MyConstants.mUploadPhotoPath = "/sdcard/LightTower/Camera"
													+ UUID.randomUUID()
															.toString();
											File file = new File(
													MyConstants.mUploadPhotoPath);
	...
	private void init() {
		...
		if (mUid == 0) {
			...
			// 通过uid查找 服务器数据库中对应的 UserMoreInfo
			// 下面只是测试数据
			mUserMoreInfo = new UserMoreInfo(301, 1, "1992-3-29", "白羊座", "西安",
					"西安", "133091818**", "安卓", "西北大学", "无");
			mUserInfo = Constants.me_UserInfo;
		...
		mSig.setText(new TextUtil().replace(mUserInfo.getSig()));
		if (mUserInfo.getSex() == 1) {
			mSex.setImageResource(R.drawable.sex_1);
		} else {
			mSex.setImageResource(R.drawable.sex_2);
		}
		// mSex.setText(Utils.getGender(mUserInfo.getSex()));
		if (mUserMoreInfo.getSingle() == 1) {
			mSingle.setText("是");
		} else if (mUserMoreInfo.getSingle() == 2) {
			mSingle.setText("否");
		} else {
			mSingle.setText("保密");
		}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case ActivityForResultUtil.REQUESTCODE_UPLOADAVATAR_CAMERA:
			if (resultCode == RESULT_OK) {
				...
				File file = new File(MyConstants.mUploadPhotoPath);
