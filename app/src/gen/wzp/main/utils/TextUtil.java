...
public class TextUtil {
	...
	private Pattern buildPattern() {
		StringBuilder patternString = new StringBuilder(
				MyConstants.mFacesText.size() * 3);
		patternString.append('(');
		for (int i = 0; i < MyConstants.mFacesText.size(); i++) {
			String s = MyConstants.mFacesText.get(i);
	...
	public CharSequence replace(CharSequence text) {
		try {
			...
			while (matcher.find()) {
				Bitmap bitmap = MyConstants
						.getFaceBitmap(MyConstants.mFacesText
